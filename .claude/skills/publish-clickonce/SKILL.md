...
1. **Update `MDEdit/samples/ReleaseNotes.md`'s "Recent changes" section first**, before running the publish script. Add a `### Version <Major.Minor.Build>` block — the *release* number only, no Revision — for the version about to be published, newest first.
   ...
   - **Then reconcile the "Known issues" and "Planned" sections in `ReleaseNotes.md` *and* the matching pair in `README.md` — every publish, not just when something obviously changed.** Both files carry these lists deliberately (see `CLAUDE.md`); updating one and not the other is the most likely way they go wrong. Drop any issue fixed in this release, add any newly-found one, and remove any planned item that just shipped. These are the two sections that rot silently: nothing fails a build or a test when they go stale, and shipping a known-issues list that still names a fixed bug is actively misleading. Known issues are user-facing defects in plain language with a workaround where one exists — not the full CommonMark-gap list from `CLAUDE.md`. Planned carries only items marked *(planned)* in `Requirements.md`, never deferred or undecided work.
